...
import {
    CollectFileExcel,
    GetExfileVpis,
    CollectFileBuildVpis,
    GetCountUploadEdms,
} from "../../common/action";

const VPISExcelUploadModalConstants = {
    GetTableInfo: () => {
        let THeader: string[] = [
            "No.",
            "Document No",
        ...
        let THeadSize: number[] = [0.6, 1, 1.5, 1, 1, 1];
        let THeadType: number[] = [1, 1, 1, 1, 1, 1];
...
export type VPISExcelUploadModalProps = {
    visible: boolean;
    onClose: () => void;
    interval: (_interval: NodeJS.Timeout) => void;
};

interface FinalVPISExcelUploadModalProps extends VPISExcelUploadModalProps {}

export const VPISExcelUploadModalComp: React.FunctionComponent<
    FinalVPISExcelUploadModalProps
> = props => {
    ...
    const tableInfo = VPISExcelUploadModalConstants.GetTableInfo();
    ...
    const [toastMsg, setToastMsg] = useState<string>(
        VPISExcelUploadModalConstants.MSG_NONE_PROJECT_ALERT
    );
    //
    ...
    useEffect(() => {
        if (fileSelector.collect_file_ex && props.visible) {
            ...
            dispatch(GetExfileVpis(fileSelector.collect_file_ex, projNo));
        }
    }, [fileSelector.collect_file_ex]);

    useEffect(() => {
        if (fileSelector.exfile_list_vpis) {
            setIsLoading(false);
            let data = fileSelector.exfile_list_vpis;
            let tableInfo = VPISExcelUploadModalConstants.GetTableInfo();
            setTableHeadType([...tableInfo.headType]);
            setTableHeadSize([...tableInfo.headSize]);
            setTableHeader([...tableInfo.header]);
            setExFileList([...data]);
        }
    }, [fileSelector.exfile_list_vpis]);
    ...
    const handleFileDrop = async (dropFiles: FileList) => {
        if (projNo == -1) {
            ...
            setToastMsg(VPISExcelUploadModalConstants.MSG_NONE_PROJECT_ALERT);
            return;
        }
    ...
    const onClickComplete = async () => {
        ...
        if (projNo && projNo != -1) {
            setIsLoading(true);
            await dispatch(CollectFileBuildVpis(fileSelector.collect_file_ex, projNo));
            // drawing interval
