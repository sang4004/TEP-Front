...
const CustomLoginTab = (props)=> {
    ...
    const onClickLogin = async(model)=>{
		if(selectedUser != -1 && onSelectUser){
			await dispatch(Login(login.user_list[selectedUser].userid, model.password ));
		} else {
	...
	//simple login blocked;
	if(false && login.user_ids && login.user_ids.length > 0 && onSelectUser){
		return (
							...
							return (
								<CustomLoginTabUserBtn $selected={selectedUser == idx} key={idx} onClick={()=>selectUser(idx)}>
									<CustomLoginTabUserImgDiv 
										style={{backgroundImage : `url(${raw.profile_img ? raw.profile_img : "assets/images/logos/1x1_logo.png"})`}} 
										/>
