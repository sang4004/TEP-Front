...
import { selectToolbarTheme } from '@fuse_app/store/fuse/settingsSlice';
...
function ToolbarLayout1(props) {
	...
	const onClickTopCode = (id)=>{
		dispatch(Setworkcode(id));
	}

	useEffect(()=>{
		if(workSelector.selectedLine){
			dispatch(Setworkcode(workSelector.selectedLine.lineNo));
		}
	}, [ workSelector.selectedLine ]);

	useEffect(()=>{
		setSelectedCode(workSelector.selectedCode);
	}, [ workSelector.selectedCode ]);
			...
			<AppBar
				...
				<Toolbar className="p-0 min-h-48 h-full">
					...
					<ToolbarTopDiv className="flex items-center px-16">
						...
						<TopNumberBtnGroup>
						{workSelector.lineCodes.map((raw,idx)=>{
							return (
								<TopNumberBtn 
									key={idx} 
									color="primary"
									selected={selectedCode == workSelector.lineNos[idx]}
									onClick={()=>onClickTopCode(workSelector.lineNos[idx])}
									>
									{raw}
								</TopNumberBtn>
							);
